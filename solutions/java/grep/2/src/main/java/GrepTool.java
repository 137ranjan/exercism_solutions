...
class GrepFlags {
    boolean lineNumbers;     // -n
    boolean listFilesOnly;   // -l
    boolean caseInsensitve;  // -i
    boolean invertMatch;     // -v
    boolean matchEntireLine; // -x

    static GrepFlags from(List<String> flags) {
        GrepFlags gf = new GrepFlags();
        for (String flag : flags) {
            switch (flag) {
                case "-n" -> gf.lineNumbers = true;
                case "-l" -> gf.listFilesOnly = true;
                case "-i" -> gf.caseInsensitve = true;
                case "-v" -> gf.invertMatch = true;
                case "-x" -> gf.matchEntireLine = true;
            }
        }
        return gf;
    }
